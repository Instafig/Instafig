...
const (
	GLISP_SYMBOL_TYPE_APP_VERSION = "APP_VERSION"
	GLISP_SYMBOL_TYPE_OS_VERSION  = "OS_VERSION"
	GLISP_SYMBOL_TYPE_LANG        = "LANG"
	GLISP_SYMBOL_TYPE_OS_TYPE     = "OS_TYPE"
	GLISP_SYMBOL_TYPE_DEVICE_ID   = "DEVICE_ID"
	GLISP_SYMBOL_TYPE_TIMEZONE    = "TIMEZONE"
	GLISP_SYMBOL_TYPE_NETWORK     = "NETWORK"
	GLISP_SYMBOL_TYPE_IP          = "IP"
	GLISP_SYMBOL_TYPE_APP_KEY     = "APP_KEY"
)
...
var glispFuncContexts = []glispFuncContext{
	...
	{"ver=",
		2,
		[]string{GLISP_SYMBOL_TYPE_APP_VERSION, GLISP_SYMBOL_TYPE_OS_VERSION},
		false,
		versionFuncCheckFunc,
	},
	{"ver>",
	...
	{"str=",
		2,
		[]string{GLISP_SYMBOL_TYPE_APP_KEY, GLISP_SYMBOL_TYPE_IP, GLISP_SYMBOL_TYPE_LANG, GLISP_SYMBOL_TYPE_DEVICE_ID, GLISP_SYMBOL_TYPE_NETWORK, GLISP_SYMBOL_TYPE_TIMEZONE, GLISP_SYMBOL_TYPE_OS_TYPE},
		false,
		stringFuncCheckFunc,
	},
	{"str!=",
...
func init() {
	for _, symbolContext := range glispSymbolContexts {
		s := symbolContext
		supportedSymbolContexts[symbolContext.typ] = &s
	}
...
func SetClientData(env *glisp.Glisp, cdata *ClientData) error {
	env.AddGlobal(GLISP_SYMBOL_TYPE_APP_KEY, glisp.SexpStr(cdata.AppKey))
	env.AddGlobal(GLISP_SYMBOL_TYPE_OS_TYPE, glisp.SexpStr(cdata.OSType))
	env.AddGlobal(GLISP_SYMBOL_TYPE_OS_VERSION, glisp.SexpStr(cdata.OSVersion))
	env.AddGlobal(GLISP_SYMBOL_TYPE_APP_VERSION, glisp.SexpStr(cdata.AppVersion))
	env.AddGlobal(GLISP_SYMBOL_TYPE_IP, glisp.SexpStr(cdata.Ip))
	env.AddGlobal(GLISP_SYMBOL_TYPE_LANG, glisp.SexpStr(cdata.Lang))
	env.AddGlobal(GLISP_SYMBOL_TYPE_DEVICE_ID, glisp.SexpStr(cdata.DeviceId))
	env.AddGlobal(GLISP_SYMBOL_TYPE_TIMEZONE, glisp.SexpStr(cdata.TimeZone))
	env.AddGlobal(GLISP_SYMBOL_TYPE_NETWORK, glisp.SexpStr(cdata.NetWork))
	return nil
}

func ClearClientData(env *glisp.Glisp) error {
	env.AddGlobal(GLISP_SYMBOL_TYPE_APP_KEY, glisp.SexpNull)
	env.AddGlobal(GLISP_SYMBOL_TYPE_OS_TYPE, glisp.SexpNull)
	env.AddGlobal(GLISP_SYMBOL_TYPE_OS_VERSION, glisp.SexpNull)
	env.AddGlobal(GLISP_SYMBOL_TYPE_APP_VERSION, glisp.SexpNull)
	env.AddGlobal(GLISP_SYMBOL_TYPE_IP, glisp.SexpNull)
	env.AddGlobal(GLISP_SYMBOL_TYPE_LANG, glisp.SexpNull)
	env.AddGlobal(GLISP_SYMBOL_TYPE_DEVICE_ID, glisp.SexpNull)
	env.AddGlobal(GLISP_SYMBOL_TYPE_TIMEZONE, glisp.SexpNull)
	env.AddGlobal(GLISP_SYMBOL_TYPE_NETWORK, glisp.SexpNull)
		...
		if val, ok := data["func"]; ok { // cond-values style function call
			...
			if !funcContext.multiArgumentsSymbol {
				...
				if len(funcContext.supportSymbols) > 0 {
					...
					if !ok {
						return "", fmt.Errorf("symbol <%s> is not supported in func <%s>", symbolContext.typ, funcContext.name)
					}
				}
			}
